...
st.write("Wir hätten sogar noch fast Flugzeuge mit reingenommen, aber der Flugzeugmodus hat die Daten manipuliert, daher müssen diese sechs erstmal reichen")
...
st.write("Unser Modell ist darauf trainiert worden, einminütige Sequenzen zu klassifizieren. Demnach ist jede Sequenz, die eine Minute lang geht, "
         "ein Datenpunkt in unserem Datenbestand. Das ergibt in Summe ungefähr 350 Datenpunkte pro Kategorie. "
...
st.write("Die Sensoren bringen alle mehrere Attribute mit sich. Da wir nicht alle von ihnen in ihrer aktuell vorliegenden Form benötigen, haben wir manche Attribute rausgeworfen und "
...
st.write("Wir haben auch versucht den Gravity Sensor mit reinzunehmen, aber bei der Feature-Importance Analyse hat sich herausgestellt, dass der Sensor kaum Auswirkungen hat."
         " Aber zu der Feature-Importance Analyse später mehr!")

st.write("")

st.subheader("Der Pipelineprozess")
st.write("Ok! Dadurch das wir wissen, welche Sensordaten wir genau benutzen, müssen wir darüber reden, was wir eigentlich genau machen wollen.")
...
st.write("Die dritte Option ist für unseren Fall keine Option gewesen, da die Sensorlogger App Aufnahmen, die eine gewissen Dateigröße überschreiten, nicht mehr als JSON, sondern nur noch als"
...
st.write("Jede Aufnahme hat ein Attribut namens seconds_elapsed. Wie der Name vermuten lässt, gibt es an, wie viele Sekunden zu jedem Datenpunkt einer Aufnahme vergangen sind. Um herauszufinden,"
         " wie lange eine Aufnahme geht, müssen wir nur den letzten Datenpunkt einer Aufnahme anschauen und dessen seconds_elapsed auslesen.")
st.write("Damit die Aufnahmen die gleiche Länge haben, schrieben wir die Methode split_data(). Sie nimmt ein Dictionary und gibt eine Liste von Dictionaries zurück, "
...
st.write("Nun müssen wir noch die Metriken über die aufgeteilten Stücke berechnen. Dieser Schritt war... ein Problem. Hierfür haben wir die Methode calculate_features() implementiert. "
         ...
         " sieht man also, zu welchem Sensor diese Metriken gehören.")

st.image("CalculateFeatures.png", caption = "Metrikberechnung mit der calculate_features Methode")

st.write("")
st.write("Jetzt ist natürlich interessant, wie wir unsere Metriken genau berechnen und warum dieser Schritt ein Problem war, denn die Implementierung war es nicht. ")
...
st.write("Und jetzt kommen wir zum Problem: Das hat richtig schlecht performt! Unseren ganzen Datenbestand zu transformieren hat >6h gedauert. Das war noch nicht Mal das größte Problem. "
...
st.write(eval)
rV_avg = (eval["Anteil an richtigen Vorhersagen"].sum() / len(eval["Anteil an richtigen Vorhersagen"]))
st.write("Der durchschnittliche Anteil an richtigen Vorhersagen beträgt hier ")
st.write(rV_avg)
st.write("Insgesamt wurden in der Validierungsphase von ")
st.write((eval["Aufnahmelänge(min)"].sum()))
st.write("Minuten so viele richtig klassifiziert: ")
st.write(eval["Absolute richtige Vorhersagen (berechnet)"].sum())